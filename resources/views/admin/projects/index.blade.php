        ...
        <div class="card">
            <div class="card-body">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap table-striped dataTable no-footer dtr-inline" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tr  id="tr_{{ $project->id }}" class="@if($project->status == 0)  deactive @endif">
                                                @can('edit-data')  
                                                ...
                                                <td class="align-middle">
                                                    <div class="custom-control custom-switch mb-2" dir="ltr">
                                                        <input type="checkbox" class="custom-control-input" id="recdis_{{ $project->id }}" @if($project->status == 1)  checked @endif value="1"  onclick="disableRecord({{ $project->id }},this);">
                                                        <label class="custom-control-label" for="recdis_{{ $project->id }}"></label>
                                                    </div>
                                                </td> 
                                                @endcan
...
<script>
    ...
    function disableRecord(id,chkbox) {
        ...
        $.ajax({
            url:"{{ route('admin.projects_status_change') }}",
            method: "POST",
