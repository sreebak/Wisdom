...
@section('pagebc')
@can('create-data') 
<a href="{{ route('admin.blgcategories.create') }}" class="btn btn-primary"><i class="fa fa-plus-circle"></i> Add New</a>
@endcan
@cannot('create-data') 
<li class="breadcrumb-item"><a href="{{ route('admin.blgcategories.index') }}">Manage Blog Categories</a></li>
            ...
            <div class="card-body">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap table-striped dataTable no-footer dtr-inline" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tr id="tr_{{ $category->id }}" class="@if($category->status == 0)  deactive @endif">
                                                ...
                                                <a href="{{ route('admin.blgcategories.edit',$category->id) }}" data-toggle="tooltip" data-placement="top" title="Edit"><i class="bx bx-edit font-size-18"></i></a>
                                                </td>
                                                @endcan
                                                @can('delete-data')
                                                <td class="align-middle">
                                                    ...
                                                    <form id="delete-form-{{ $category->id }}" action="{{ route('admin.blgcategories.destroy',$category->id) }}" method="POST" style="display: none;">
                                                        @csrf
                                                        {{ method_field('DELETE')}}
                                                    </form>
                                                </td>
                                                ...
                                                <td class="align-middle">
                                                    @if($category->image)
                                                    <a href="{{ url('../storage/blgcatg/'.$category->image) }}" target="_blank"><img src="{{ url('../storage/blgcatg/'.$category->image) }}" class="img-thumbnail" style="width: 50px;"></a>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endforeach
...
<script>
    ...
    function disableRecord(id,chkbox) {
        ...
        $.ajax({
            url:"{{ route('admin.blgcatg_status_change') }}",
            method: "POST",
