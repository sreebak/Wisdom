        ...
        <div class="card">
            <div class="card-body">
                ...
                <table id="datatable" class="table table-bordered dt-responsive nowrap table-striped dataTable no-footer dtr-inline" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tr id="tr_{{ $gallery->id }}" class="@if($gallery->status == 0)  deactive @endif">
                                                @can('edit-data')  
                                                ...
                                                <td class="align-middle">
                                                    <div class="custom-control custom-switch mb-2" dir="ltr">
                                                        <input type="checkbox" class="custom-control-input" id="recdis_{{ $gallery->id }}" @if($gallery->status == 1)  checked @endif value="1"  onclick="disableRecord({{ $gallery->id }},this);">
                                                        <label class="custom-control-label" for="recdis_{{ $gallery->id }}"></label>
                                                    </div>
                                                </td> 
                                                @endcan
...
<script>
    ...
    function disableRecord(id,chkbox) {
        ...
        $.ajax({
            url:"{{ route('admin.gallery_status_change') }}",
            method: "POST",
